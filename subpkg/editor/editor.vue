<template>
	<view class="container">
		...
		<view class="editor">
			<richText ref="richText" :readOnly="readOnly" :placeholder="placeholder" formatDate="YY/MM/DD"
				buttonTxt="保存" @clearBeforeEvent="clearBeforeEvent($event, { tagId: 'richText' })"
				@clearSuccess="clearSuccess($event, { tagId: 'richText' })" @undo="undo($event, { tagId: 'richText' })"
				...
				@getEditorContent="getEditorContent($event, { tagId: 'richText' })"></richText>
			<!--<view class="tip">
			    备注：
	...
	export default {
		...
		methods: {
			...
			async addData() {
				...
				const {
					data: res
				} = await uni.$http.post("/addPaper", {
					author: this.userinfo.nickName,
					content: app.globalData.data.richTextContents,
					img: this.coverimg,
					paperType: this.array[this.index],
					title: this.title,
			...
			upImgToDB(path) {
				...
				// })
				uni.uploadFile({
					url: 'http://127.0.0.1:7070/upPaperTitleImage',
					filePath: path,
					name: 'image',
			...
			insertImageEvent(e, _dataset) {
				...
				uni.chooseImage({
					...
					success: (res) => {
						...
						// 	})
						uni.uploadFile({
							url: 'http://127.0.0.1:7070/upPaperInnerImage',
							filePath: path,
							name: 'image',
							success: (uploadFileRes) => {
								console.log(uploadFileRes);
								...
								let startIndex = jsonString.indexOf(startString);
								// 找到结束字符串的位置
								let endIndex = jsonString.indexOf(endString, startIndex +
									startString.length);
								let result = jsonString.substring(startIndex + startString.length,
									endIndex);
								this.$refs.richText
									.insertImageMethod(result)
									.then((res) => {
										console.log('[insert image success callback]=>', res);
									})
									.catch((res) => {
