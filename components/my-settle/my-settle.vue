	...
	export default {
		...
		methods: {
			...
			async payOrder() {
				...
				// const orderDetail = {
				// 	// 开发期间，注释掉真实的订单价格，
				// 	orderCount: this.checkedGoodsAmount,
				// 	// 写死订单总价为 1 分钱
				// 	//order_price: 0.01,
				// 	address: this.address,
				// 	orderDetail: this.cart.filter(x => x.goods_state).map(x => ({
				// 		id:x.id,
				// 		goodsName: x.goodsName,
				// 		goodsNumber: x.goodsNumber,
				// 		goodsPrice: x.goodsPrice,
				// 		goodsLogo: x.goodsLogo,
				// 	})),
				// }
				const {data:res}
				...
				uni.navigateTo({
					url: '/subpkg/order-detail/order-detail?id='+ orderNum
				});
				var delGoodsList = this.cart.filter(x => x.goods_state).map(x => ({
					id: x.id
				}));
				console.log(1)
				console.log(delGoodsList)
				for (var i = 0; i < delGoodsList.length; i++) {
					this.removeGoodsById(delGoodsList[i].id)
				}
			},
			//  getCurrentDateTime() {
			// const now = new Date();
